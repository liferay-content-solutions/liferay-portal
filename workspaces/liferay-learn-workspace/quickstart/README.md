...
3. From within the quickstart directory of the Liferay Learn workspace, run `./run.sh`
...
3. Deploy every extension by running: `../gradlew clean deploy "-Ddeploy.docker.container.id=liferay"`.
4. Extensions can also be deployed one by one from each of their respective directories by running:
...
3. Click on `Liferay Learn Etc Cron Oauth Application Headless Server` and copy both `Client ID` and `Client Secret`.
4. Export the following environment variables in your terminal (replacing the values with appropriate content from prior
   steps).
...
5. In your Liferay Portal Repo, navigate
   to `workspaces/liferay-learn-workspace/client-extensions/liferay-learn-etc-cron`.
6. From the Liferay Learn ETC Cron directory run `../../gradlew run`.